...
@interface CXImageCropper ()<UIScrollViewDelegate>{
    CGFloat _selfHeight;
    CGFloat _selfWidth;
    CGRect _cropFrame;
}
...
-(void)cancleButtonClick{
    if ([self.delegate respondsToSelector:@selector(cxImageCroppingDidCancle:)]) {
        [self.delegate cxImageCroppingDidCancle:self];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
}

-(void)sureButtonClick{
    if ([self.delegate respondsToSelector:@selector(cxImageCropping:didCropImage:)]) {
        [self.delegate cxImageCropping:self didCropImage:[self getSubImage]];
    }
    [self dismissViewControllerAnimated:YES completion:nil];
}
