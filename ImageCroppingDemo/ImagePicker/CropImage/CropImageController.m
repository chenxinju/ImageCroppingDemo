...
- (void)userInterface {
    ...
    UIView * view = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.bounds.size.height - 46-Height_TapBarSafeBottomMargin, self.view.bounds.size.width, 46+Height_TapBarSafeBottomMargin)];
...
- (void)doneBtnClick{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(cropImageDidFinishedWithImage:)]) {
        [_delegate cropImageDidFinishedWithImage:cropImage];
    }
    
    if (self.cropImageDidFinishedWithImageBclock) {
        self.cropImageDidFinishedWithImageBclock(cropImage);
    }
    if (self.isPush) {
        [self.navigationController popViewControllerAnimated:YES];
    }else {
